...
**Content**:
Rich text — the result of block-based editing. Not a place on the Entity but a **structured Data Type** (`core.rich-content`), so an Entity has prose only where a **Field** of that Data Type is present — defaulted by a **Type** or attached directly — and may have more than one (a deity's public `core.content` and its `world.secrets`). The canonical prose Field lands at the `core.content` key. Shipped by a bundled plugin, like the **Hex Map**: an Instance without it has no editor to open one on.
