...
E2E is a **small, curated regression net** over the cross-process _seams_ — cookie auth, the HTTP round trip, and persistence across reload — not a per-feature acceptance suite (domain logic stays covered by the vitest specs). The runner is **Playwright** (chromium only) in a new `apps/web-e2e` project, run via `nx e2e web-e2e`.

Tests run against the **real production build**: the api serves the built SPA on a **single origin** (ADR-0008), so Playwright's `webServer` boots one Nest process (`dist/apps/api/main.js` serving `dist/apps/web/browser`) and tests hit one port. There is no separate proxy and no two-origin/CORS setup — the harness _is_ the prod topology.
