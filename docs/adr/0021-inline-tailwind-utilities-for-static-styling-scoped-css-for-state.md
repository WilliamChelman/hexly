...
> **Extended by [ADR-0031](./0031-apply-in-scoped-styles-for-translatable-props.md):** the hybrid seam also applies _inside_ a primitive's scoped `styles:` block via `@apply`, not only as inline-template utilities — translatable props convert, the custom core stays raw.
...
still _own_ their visual definition and there is still no global component-class
...
tokens _be_ the Tailwind theme: `bg-surface`, `text-ink-muted`, `gap-5`,
...
theme namespace inlines a `shadow-*` utility's _value_ rather than referencing
...
leave a _minimal_ scoped rule only for the ones that genuinely can't — keeping
...
- **Elevation → raw tokens wrapped in a custom `@utility`.** Shadows are the one
  place "a `@theme` token both re-themes _and_ generates a utility" (ADR-0020)
  breaks down: to leave room for `--tw-shadow-color`, Tailwind inlines a
  `shadow-*` utility's _value_ rather than referencing `var(--shadow-*)`, so a
  ...
  buys nothing — so the `--shadow-*` light values move _out_ of `@theme` into
...
var(--shadow-1); }`. Call sites keep the clean `shadow-1` name, a single rule
  ...
  `text-shadow-*` inline their geometry identically — so a _themed_ drop- or
...
- **These keep the whole element scoped** (the genuinely-can't cases):
  ...
  - **Class-toggle state** (`.is-active`, `.option.active`, `.toast.is-error`) —
    an element styled by its _own_ toggled state. This is usually still
    expressible _without_ scoped CSS, and the preferred order is:
  ...
  - **Transitions/animations** — motion is kept as raw `--dur-*`/`--ease-*` vars
    ...
    scoped. When the _base_ rule stays scoped for this reason, its sibling
  ...
  - **Reaching into another primitive's host** — a consumer cannot reliably
    ...
    _unlayered_ and so beat any `@layer utilities` rule regardless of source
...
- **Split a mixed rule only when the two halves set different properties.** When
  ...
  ones and leave a _minimal_ scoped rule for the remainder — provided the inline
  ...
  Keep the whole rule scoped when the two halves would set the _same_ property
...
transparent)` stays scoped. `bg-surface/86` would express the same fill, but
  the opacity modifier emits _two_ declarations — a baked-srgb fallback
...
- **One rule, one home.** The deciding test is per-rule, not per-component: a
  component routinely ends up with a `host: { class }` _and_ a residual
...
- Most atoms and view shells shed their `styles:` block entirely (the icon
  ...
  at a glance and the `styles:` block is now _only_ its interesting,
