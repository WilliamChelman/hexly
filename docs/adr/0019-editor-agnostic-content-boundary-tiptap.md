...
Adding or removing a schema-affecting extension (a node or mark) changes what loads without dropping, so it is a `format` bump (`tiptap-v2`, …) + migration — **not** a transparent edit. Non-schema extensions (input rules, keymaps, history) may change freely. The planned slash menu, formatting toolbar, and custom blocks (hex-map and entity-reference node views) extend this set behind the same contract.
