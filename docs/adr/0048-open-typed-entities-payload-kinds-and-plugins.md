...
The *Views* amendment left the save seam as a narrow **`GridStore` port** — load / working-document / editable — that `HexMapStore` implemented and the session drove. That fit one editable payload (the grid), but every View that #187 brings (stat-block, the generic Field view, Metadata) edits a *different slice of the same one Entity body*. A port per slice, each pulled by the session, doesn't generalize; the coupling is the wrong way round.
