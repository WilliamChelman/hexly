...
The read predicate takes a third disjunct beside `grant` and `member ∧ shared`: _the caller reaches the Entity's Container, **or is a member of** some World that **Mounts** it_ — one `EXISTS`, one hop, since Mounts do not chain. Membership, not reachability: a caller who reaches a World only by an Entity grant inside it cannot read that World's own `shared` content either, so a Mount must not hand them more _through_ the Mount than beside it. This is the first time reading a Container depends on another Container's configuration, which is what un-cements the "cemented" of the title.

The escalation guard is that only a Container you **Own** may be mounted, so a Mount republishes nothing you do not already control — and because a co-Owner may evict any Owner (above), that rule is asked **on every read**, not only when the Mount is declared: the cascade holds while some Owner of the mounting World still Owns the mounted Container, and stops the moment none does. A **Compendium** is exempt, being Instance-wide with no members. Reachable is not the same as **listed**: a World reached only through a Mount is readable and absent from the World Index, the Switcher and quick-open, which answer "the Worlds you have". The cascade reaches anonymous **World Public Link** holders too, deliberately.
