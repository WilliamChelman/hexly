...
- **Transloco over ngx-translate.** Both are runtime JSON i18n libraries; Transloco wins on first-class lazy **scopes**, a maintained **signal API** (`translateSignal` / `translateObjectSignal`) that fits this **zoneless Angular 21** app, and better tooling (`@jsverse/transloco-keys-manager` for scaffolding and missing/orphaned-key validation). We lean on the signal API where reactivity matters rather than the legacy pipe/structural directive.
...
- **What is *not* translated.** User-typed **content** — Region names, Labels, Notes, map titles — is data and is never translated. Built-in **catalog labels** (terrain/feature `label`s in the framework-agnostic domain lib) *are* translated, but at the **UI layer keyed by their stable `id`** (`domain.terrain.grass`, `domain.feature.settlement`); the domain `label` stays as the English default/fallback and the domain lib never imports Transloco. Canonical glossary nouns (Region, Hex, Terrain…) are translated to natural French; the product name **"Hexly" stays**. A parallel **French glossary** is maintained as the source of truth for those nouns so translations stay consistent with `CONTEXT.md`.
...
- **CI gains a key-sync gate** via `transloco-keys-manager` that fails on missing/orphaned keys, keeping `en.json` and `fr.json` from drifting. No brittle "no hardcoded string" lint rule for now — the keys-manager diff plus review is the guard.
- **A new French glossary doc** sits alongside `CONTEXT.md` as the canonical French rendering of the domain nouns; `fr.json` must not become the de-facto dictionary.
