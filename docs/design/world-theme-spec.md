...
**Token resolution test.** Read `getComputedStyle(document.documentElement)` for every _declared_ token (not only the registered ones) in both ColorSchemes, and assert each one resolves. This is the only net for the "Solar and Astral go through the derivation path" condition. It also exercises the exact resolved-value path the contrast reporter depends on, so the test and the mechanism are the same thing.

It ships as `apps/web-e2e/src/design-tokens.spec.ts`. While the derivation work was in flight it also asserted every token's resolved value against a committed table, whose diff was the list of colours that work moved; the table was dropped once the work landed, past which point it detected intended palette edits and nothing else. What survives is what no static reader can compute: that every registered token resolves rather than reading back as its raw declaration, that none reads back still carrying a `var()`, and that each colour's manifest `initial-value` — the fallback three Canvas renderers take — is what its expression actually resolves to.

**Nine public tokens sit outside the shape check.** `--shadow-1/2/3/inset/focus` and the four `--font-*` cannot be `@property`-registered — Properties & Values has no shadow or font-stack syntax component — so they read back as the token stream the stylesheet wrote. The spec asserts the one thing it can, that nothing reads back still carrying a `var()`. §2.2's re-parenting of `shadow-2/3/inset` onto an ink anchor therefore survives only as whatever expression substitution leaves, which is legitimate for a `box-shadow` (the engine evaluates it at use) and would not be for anything a Canvas renderer reads. None of the nine is.
