 ...
 */
test('the preload bridge offers a session re-mint, and nothing else', async ({ launch }) => {
  const { window } = await launch();

  // One member wide: the bridge is main's whole surface to the renderer, and its presence is what the
  ...
  expect(members).toEqual(['renewSession: function']);
});
