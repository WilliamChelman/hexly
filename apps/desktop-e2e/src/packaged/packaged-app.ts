...
/** Where electron-builder puts things and what it names them (#327), in one place. */
...
/**
 * The unpacked app beside that installer, composed rather than searched for so a run that finds nothing says
 * which path it wanted; the builder config pins the same host arch this resolves.
 */
...
/**
 * A package carries its own Electron, `node_modules` and SPA, so it needs nothing from the working tree —
 * including whichever ABI that `node_modules` currently holds.
 */
