...
/**
 * The Electron suite's base test. `launch` opens the Desktop App against one throwaway Instance Directory
 * per test — call it twice for a relaunch — and quits every run it opened at teardown.
 */
export const test = base.extend<{ userDataDir: string; launch: () => Promise<DesktopRun> }>({
  ...
  launch: async ({ userDataDir }, use) => {
    ...
    await use(async () => {
      const run = await launchDesktopApp(userDataDir);
      runs.push(run);
      return run;
    });
 ...
 */
async function launchDesktopApp(userDataDir: string): Promise<DesktopRun> {
  requireBuilt(mainJs, "the Desktop App's build");
  requireBuilt(webIndex, "the web app's build");
  ...
  try {
    ...
    throw new Error(bootFailureMessage(err, output()));
  }
}
...
export async function launchAgain(userDataDir: string): Promise<SecondLaunch> {
  const child = spawn(electronBinary, launchArgs(userDataDir), { cwd: workspaceRoot });
...
function launchArgs(userDataDir: string): string[] {
  return [mainJs, `--user-data-dir=${userDataDir}`];
}
...
function bootFailureMessage(err: unknown, mainOutput: string): string {
  return [
