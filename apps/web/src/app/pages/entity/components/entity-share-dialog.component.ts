...
import { GrantSetComponent, OwnerSetComponent } from '@hexly/web-entity';
import { ClientConfigStore } from '@hexly/web-core';
import { EntitySession } from '../services/entity-session';

/**
 * The open Entity's Share dialog (ADR-0037): the owner set and named per-Entity grants — the
 * owner-only sharing surface. Driven by the {@link open} input; its content mounts only while open,
 * so it never fetches owners/grants for a closed dialog. {@link resigned} fires when the caller
 * resigns ownership, which can cost reach to the Entity — the caller navigates away.
 *
 * The surface guards itself on the Collaboration layer (ADR-0071), so no host can mount its parts
 * whose endpoints 404.
 *
...
@Component({
  ...
  host: { class: 'contents' },
  imports: [ButtonComponent, DialogComponent, GrantSetComponent, OwnerSetComponent, TranslocoPipe],
  template: `
    @if (open() && collaboration() && entityId(); as id) {
      <app-dialog [open]="true" [heading]="'collab.owners.heading' | transloco" (closed)="closed.emit()">
        ...
        <!-- Named per-Entity grants (ADR-0037, #161): the surgical layer below ownership — hand a
             specific user Editor/Viewer on just this Entity, piercing private. The successor to the
             retired per-Entity Public Link's named case (ADR-0084). -->
