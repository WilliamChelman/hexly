...
The bundled Board plugin (CONTEXT.md → Board, ADR-0062, #263): the `core.board` Entity Type, the
`core.board-surface` **Structured Data Type** that _is_ its surface, and (in the `-web` half) the canvas
...
The ids keep the `core.` namespace (`core.board`, `core.board-surface`, `core.surface`) though the code
