...
The bundled Hex Map plugin (CONTEXT.md → Hex Map, ADR-0050, #198): the `core.type.hex-map` Entity Type, the
`core.datatype.hex-grid` **Structured Data Type** that _is_ its grid, and the canvas that edits it. The
...
- `@hexly/plugin-hexmap` — framework-free: the hex geometry (coordinates, layout, edges, culling,
  marquee, move-planner), the grid document (`hexMapSchema`), the `core.datatype.hex-grid` data-type (its
  schema, its empty plane, its edge harvester), and the `core.type.hex-map` `defineType()`. The API names it
...
The ids keep the `core.` namespace (`core.type.hex-map`, `core.datatype.hex-grid`, `core.view.map`) though the code
