/**
 * `core.datatype.hex-grid` — the Hex Map's grid as a **Structured Data Type** (CONTEXT.md → Structured Data
 * Type, ADR-0050). `core.type.hex-map` declares it at the `grid` key, so a Hex Map's plane lives in the
...
export const CORE_HEX_GRID: StructuredDataTypeId = 'core.datatype.hex-grid';
...
export const HEX_GRID_FIELD_ID = 'core.field.grid';

/**
 * The Field `core.type.hex-map` references, and the EntityDocument slice the map editor reads and writes — a
 ...
 *
 * Its `id` (`core.field.grid`) *is* the EntityDocument slot it lenses — one namespaced identifier (ADR-0056).
 *
