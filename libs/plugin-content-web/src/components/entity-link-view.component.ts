...
export interface EntityLinkRepair {
   ...
   * the URL's (ADR-0073), so what a repair may point at and what it may create are one answer. Unset
   * before the surface has resolved one, which leaves the search unscoped but still sealed: a
   * link-target read returns no **Compendium Entry** either way (ADR-0079).
   */
