...
@Component({
  ...
  template: `
    ...
    @for (row of rows(); track $index) {
      <button
        type="button"
        class="w-full py-1 pr-2 text-sm text-left text-ink-muted truncate bg-transparent border-none rounded cursor-pointer hover:bg-accent-soft hover:text-ink aria-current:text-ink-strong aria-current:font-medium "
