...
export default [
  ...
  ...baseConfig,
  {
    ...
    rules: {
      // ADR-0020 — every var(--…) must resolve to a defined token; built-in shadow
      ...
      // Element components stay kebab-case. A component may also attach to a native
      // element via an attribute selector, so the primitive keeps that element's
      // semantics/a11y (ADR-0007); those are camelCase, like a directive.
