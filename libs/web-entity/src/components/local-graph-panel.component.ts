    ...
    <div class="flex items-center justify-between gap-2">
      ...
      <div class="flex items-center gap-0.5" role="group" [attr.aria-label]="'fields.localGraph.depth' | transloco">
        @for (depth of store.depths; track depth) {
          <button
            type="button"
            class="min-w-6 rounded-sm px-1.5 py-0.5 font-sans text-xs text-ink-muted hover:bg-surface-sunken aria-pressed:bg-accent/15 aria-pressed:text-accent-strong aria-pressed:font-semibold"
