...
export class EntitySearchPickerComponent {
  ...
  constructor() {
    ...
    effect((onCleanup) => {
      const sub = this.entitiesClient.facets({ ...this.read(), compendium: undefined }).subscribe({
        next: (facets) => this.containers.set(facets.compendium ?? []),
  ...
  private read() {
    ...
    return {
      ...
      compendium: container ? [container] : undefined,
