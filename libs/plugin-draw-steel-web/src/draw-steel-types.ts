 ...
 * away from the type id (`draw-steel.monster`), the Field data-type id (`draw-steel.stat-block`), and the
 * Field id (`draw-steel.stat_block`) (ADR-0050).
 */
...
export const DS_TYPE_DEFINITIONS: readonly TypeDefinition[] = [
  {
    ...
    // The crossed-swords glyph, so a Draw Steel monster reads apart from a `dnd.monster`'s skull (#242).
    icon: 'swords',
    // The stat block placed by its `{ field }` — the `draw-steel.stat-block` data-type's View, labelled by
    // the Field (ADR-0055) — then the content View by id, so a monster opens on its stats with a Note toggle.
    views: [{ field: DS_STAT_BLOCK_KEY }, CORE_VIEW_CONTENT],
    // The secondary role from the palette (docs/design/identity.md); distinct from `dnd`'s `--color-astra`.
    graphColorToken: '--color-sea',
