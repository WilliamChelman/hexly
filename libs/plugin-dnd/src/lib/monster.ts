/**
 * The `dnd.monster` bundled Plugin type (CONTEXT.md → Type Definition). A monster references two Fields:
 * its prose `core.content` and its `dnd.stat-block` — the stat block being a **Structured Data Type**
...
/**
 * The bundled `dnd.monster` type. References its two Fields by id (`fieldRefs`, ADR-0054): the prose
 * `core.content` and the `dnd.stat-block` structured Field. The stat block's harvested dimensions
