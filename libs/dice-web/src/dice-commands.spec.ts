...
/** The options the Provider raises every roll toast with — the total headlined, anchored up, lingering. */
const rollToastOpts = (total: string) => ({ title: total, placement: 'top', durationMs: DICE_TOAST_DURATION_MS });
...
describe('DiceCommands', () => {
  ...
  it('flashes the formatted Roll Result through the toaster when the Command runs', async () => {
    ...
    command.run();
    expect(show).toHaveBeenCalledWith('2d10 + 3 → 2d10: 7, 4', 'info', rollToastOpts('14'));
  });
  ...
  it('produces a fresh Roll each time the Command runs', async () => {
    ...
    command.run();
    command.run();
    expect(show).toHaveBeenNthCalledWith(1, '2d10 + 3 → 2d10: 7, 4', 'info', rollToastOpts('14'));
    expect(show).toHaveBeenNthCalledWith(2, '2d10 + 3 → 2d10: 2, 9', 'info', rollToastOpts('14'));
  });
});
