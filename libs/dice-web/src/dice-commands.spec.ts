...
describe('DiceCommands', () => {
  ...
  it('flashes the formatted Roll Result through the toaster when the Command runs', async () => {
    ...
    command.run();
    expect(show).toHaveBeenCalledWith('2d10 + 3 → 2d10: 7, 4 = 14');
  });
  ...
  it('produces a fresh Roll each time the Command runs', async () => {
    ...
    command.run();
    command.run();
    expect(show).toHaveBeenNthCalledWith(1, '2d10 + 3 → 2d10: 7, 4 = 14');
    expect(show).toHaveBeenNthCalledWith(2, '2d10 + 3 → 2d10: 2, 9 = 14');
  });
});
