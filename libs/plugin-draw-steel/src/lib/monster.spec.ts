...
describe('draw-steel.monster', () => {
  ...
  it('resolves the stat-block Field through the shared types[] → Fields path both sides ride', () => {
    ...
    expect(validateFields(fields, { 'draw-steel.stat_block': { level: 3, role: 'brute' } }, dataTypes).ok).toBe(true);
    expect(validateFields(fields, { 'draw-steel.stat_block': { might: 'strong' } }, dataTypes).ok).toBe(false);
