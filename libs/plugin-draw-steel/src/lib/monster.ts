/**
 * The `draw-steel.type.monster` bundled Plugin type (CONTEXT.md → Type Definition). A monster references two
 * Fields: its prose `core.field.content` and its `draw-steel.datatype.stat-block` — the stat block being a **Structured
...
export const DS_MONSTER = 'draw-steel.type.monster';

/**
 * The bundled `draw-steel.type.monster` type. References its two Fields by id (`fieldRefs`, ADR-0054): the
 * prose `core.field.content` first, then the `draw-steel.datatype.stat-block` structured Field — the web places the
