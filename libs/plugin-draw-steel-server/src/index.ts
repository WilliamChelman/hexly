 ...
 * reads for validation and contributes the `draw-steel.importer.monsters` **Importer** (ADR-0060), never the
...
export function serverPluginDrawSteel(): ServerPlugin {
  // Declares the `draw-steel.datatype.stat-block` structured Field and its Data Type — the source the API's
  // forward-only gate resolves a Monster's block against (ADR-0050/0055). The prose `core.field.content` it also
  // references is the content plugin's, folded from there. Contributes the `draw-steel.importer.monsters` Importer
